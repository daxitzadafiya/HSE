    ...
    <v-dialog v-model="openPopup" max-width="800px" persistent>
      <v-card>
              ...
              <b-col cols="6">
                <div class="input-wrap">
                  <v-select
                    ...
                    :items="!editMode ? availableCompanyArray : companyArray"
                    item-text="name"
                    item-value="id"
                    :label="$t('form.company')"
                    :rules="required"
                    :disabled="editMode || (!editMode && newCompany > 0)"
                    outlined
                  />
                </div>
              </b-col>
        ...
        <v-card-actions class="popup-footer">
          ...
          <v-btn
            v-if="!editMode || (editMode && editedItem.status !== 'active')"
            class="btn-save"
            @click="save"
          >
            {{ $t('button.save') }}
...
<script>
  ...
  export default {
    ...
    methods: {
      addNew() {
        this.reset();
        this.formTitle = this.$i18n.t('title.new_user');
        this.$store.dispatch('companies/index', {checkUser:true}).then(result => {
          this.availableCompanyArray = result.data;
          if (this.newCompany !== null) {
            this.editedItem.company_id = this.newCompany;
          }
        }).catch((error) => {
      ...
      editItem(item) {
        ...
        that.formTitle = that.$i18n.t('title.edit_user');
        that.$store.dispatch('users/show', item.id).then(result => {
      ...
      save() {
        ...
        if (that.$refs.form.validate()) {
          ...
          // update
          if (that.editMode) {
            that.$store.dispatch('users/update', that.editedItem).then(function () {
              that.reloadList();
              that.$toaster.success(that.$i18n.t('message.success_user_update'));
            }).catch((error) => {
            if (error.response.status == 401) {
            this.$router.push('/login');
            this.$toaster.error(this.$i18n.t('message.token_expired'));
            ...
            // create
            that.$store.dispatch('users/store', that.editedItem).then(function () {
              that.reloadList();
              that.$toaster.success(that.$i18n.t('message.success_user_add_new'));
            }).catch((error) => {
            if (error.response.status == 401) {
            this.$router.push('/login');
            this.$toaster.error(this.$i18n.t('message.token_expired'));
