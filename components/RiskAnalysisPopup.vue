            ...
            <b-row>
              <b-col cols="9">
                <div class="input-wrap">
                  <v-text-field v-model="editedItem.name" :label="$t('form.subject')" :rules="required" :readonly="editMode" outlined />
                </div>
              </b-col>
              <b-col cols="3">
                <div class="input-wrap">
                  <v-text-field
                    :value="editedItem.status ? formatStatus(editedItem.status) : 'New'"
                    :label="$t('form.status')"
                    outlined
                    readonly
                  />
...
<!--              />-->
            ...
            />
            <div v-if="!viewMode && editMode && editedItem.need_to_process && (checkAdmin || permissionsUser.indexOf('risk analysis-basic') > -1)" class="manage-task pt-4">
...
<style scoped>
:deep(.source-plus-icon .manage-task .plus-icon){
  margin-left: -86px ;
}
</style>
