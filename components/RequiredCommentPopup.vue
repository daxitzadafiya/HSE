      ...
      <v-card>
        ...
        <v-card-text>
          ...
          >
            ...
            <div v-if="objectItem?.isResponsible || objectItem?.isAttendee">
              <!-- Comment -->
              <div v-if="checkComment" class="section-connect-to">
                <div class="section-connect-to-label">
                  {{ $t('form.attached_note') }}
                </div>
                <div class="input-wrap form-description p-3">
                  <v-textarea
                    v-if="objectItem.isAttendee"
                    v-model="objectItem.comment"
                    outlined
                    :rules="required"
                  />
                  <v-textarea
              ...
              <!-- Attachment -->
              <div v-if="checkAttachment" class="section-connect-to mt-8">
                <div class="section-connect-to-label">
                  {{ $t('form.attached_image') }}
                </div>
                <div class="required-upload-attachment p-4">
...
<script>
  ...
  export default {
    ...
    methods: {
      addNew() {
        this.checkComment = this.objectItem?.requiredComment;
        this.checkAttachment = this.objectItem?.requiredAttachment;
        if (this.objectItem.comment) {
          this.objectItem.comment = this.objectItem?.comment.replace(/<[^>]+>/g, '');
        } else if (this.objectItem.responsible_comment) {
          this.objectItem.responsible_comment = this.objectItem?.responsible_comment?.replace(/<[^>]+>/g, '');
        }
      },
